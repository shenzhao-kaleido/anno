...
def adaptContrastEnhancement(scr, winSize, maxCg):
    # scr is in cv2 BGR format, try: src = cv2.cvtColor(mriNPArray.astype(np.uint8),cv2.COLOR_RGB2BGR)
    # I set winSize to 15 and maxCg 10, you can use other values 
    if scr is None or winSize is None or maxCg is None:
        print("The input parameters of ACE Function error")
        return -1
    
    YUV_img=cv2.cvtColor(scr,cv2.COLOR_BGR2YUV)   
    Y_Channel = YUV_img[:,:,0]
